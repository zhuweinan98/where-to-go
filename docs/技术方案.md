...
| **Agent 层** | 可选 LLM；否则关键词与规则；组装回复 | `src/agent/bot.py` 的 `chat()`、`src/agent/langchain_fc.py` |
...
flowchart LR
  ...
  W --> A
  A --> D
  A -.->|LLM_MODE=ollama/openai| L1
  L1 --> T1
  L1 --> T2
  L1 --> O
```

说明：`LLM_MODE=off` 仅返回兜底提示；`LLM_MODE=ollama/openai` 时统一进入 LangChain FC 路径。

## 和风天气 QWeather（可选）

- **触发条件**：同时配置环境变量 `QWEATHER_HOST`（控制台中的 API Host，可写 `https://xxx.re.qweatherapi.com` 或省略协议由代码补全）与 `QWEATHER_KEY`（控制台凭据里 **API KEY** 方式生成的 Key）。未配置或请求失败时 **`get_weather_for_city()` 回退 Mock**，行为与早期 Week 1 一致，便于单测与离线开发；开启 `LLM_DEBUG=1` 时可在终端看到 `[qweather]` 调试日志。
...
- **默认行为**：不设或 `LLM_MODE=off` 时，返回兜底提示（不走 LLM）；`pytest` 依赖此默认，**CI 务必保持 off**。
...
1. `chat()` 在 `LLM_MODE=ollama/openai` 时统一走 `src/agent/langchain_fc.py::chat_with_tools`。
...
4. FC 内部按 `LLM_MODE` 读取模型配置；`ollama/openai` 均走 OpenAI 兼容 `/v1` 协议。
5. 当前城市策略为“优先使用表单城市（默认上海）”；用户文本中的城市词不再覆盖该默认值。
...
- `LLM_MODE=off` 保持为兜底模式，单测/CI 仍建议保持该模式。
- Week 2 的 LLM 路径已统一为 LangChain FC：由模型决定是否调用工具，而非业务层预取所有数据。
...
改 **Agent / RAG / 天气 / Web 请求链** 时须同步：

1. **终端**：`LLM_DEBUG=1` 时沿用 `[llm]` / `[rag]` 等 stdout 习惯；RAG 关键步骤写入 `_rag_debug` 缓冲。
2. **通俗中文 + 专业词**：风格对齐 **`fc_start`** 例：`✅ FC启动成功：mode=ollama model=qwen3:4b 默认城市=盘锦` —— **人话 + 少量 `key=value` + 术语**（如 embedding、向量、余弦、rerank）；避免整屏无结构参数，默认不写 verbose 长文。
3. **`debug_trace`**：经 `langchain_fc` 的 `_add_trace` / `trace_hook` 进入前端调试；`list_places` / `search_sanguo_places` 执行后 **`consume_rag_debug_logs()`** 并挂到 **`tool_result.rag_debug_logs`**（工具异常时亦应 consume，避免缓冲残留）。
4. **前端**：`src/templates/index.html` 中若 trace 结构有变，同步更新 **`formatTraceItem` / `summarizeToolResult`**；`tool_result.result` 截断须保证 JSON 可解析或走摘要。
5. **CI**：默认 `LLM_MODE=off` 的 `pytest` 须仍通过。

持久给 Cursor 的说明见仓库 **`.cursor/rules/observability-and-debug-trace.mdc`**（本仓库 `alwaysApply: true`，会话内默认带上）。
